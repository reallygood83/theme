...
export function ShareSessionModal({ session, trigger, isOpen: externalIsOpen, onClose, onShareSuccess }: ShareSessionModalProps) {
  const [isSharing, setIsSharing] = useState(false);
  const [shareType, setShareType] = useState<'public' | 'restricted'>('public');
  const [tags, setTags] = useState<string[]>([]);
  const [category, setCategory] = useState('general');
  const [targetGrade, setTargetGrade] = useState<'1-2학년' | '3-4학년' | '5-6학년'>('3-4학년');
  const [internalIsOpen, setInternalIsOpen] = useState(false);
  
  // 외부에서 제어하는 경우 external props 사용, 그렇지 않으면 내부 state 사용
  ...
  const handleShare = async () => {
    console.log('🔥 handleShare 함수 호출됨:', { session: session?.id, title: session?.title });
    
    if (!session) {
      console.log('❌ session이 없음, 공유 중단');
      return;
    }

    console.log('📤 세션 공유 시작 - API 호출 예정');
    setIsSharing(true);
    
    try {
      // 30-Second Reality Check: 실제 API 호출
      console.log('🚀 API 호출 시작: /api/shared/sessions/create');
      ...
      console.log('✅ 세션 공유 성공:', result.sharedSessionId);
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                {(['1-2학년', '3-4학년', '5-6학년'] as const).map((grade) => (
                  <button
                    key={grade}
                    type="button"
